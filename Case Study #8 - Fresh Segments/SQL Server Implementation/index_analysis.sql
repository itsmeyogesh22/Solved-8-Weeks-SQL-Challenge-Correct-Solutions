...
WITH cte_index_composition AS (
  SELECT
    ...
    ((interest_metrics.composition/interest_metrics.index_value)) AS index_composition,
    RANK() OVER (
...
SELECT
  ...
  LAG(CONCAT(interest_name, ': ', ROUND(index_composition, 2))) OVER (ORDER BY month_year) AS "1_month_ago",
  LAG(CONCAT(interest_name, ': ', ROUND(index_composition, 2)), 2) OVER (ORDER BY month_year) AS "2_months_ago"
FROM cte_index_composition
WHERE index_rank = 1
)
SELECT *
