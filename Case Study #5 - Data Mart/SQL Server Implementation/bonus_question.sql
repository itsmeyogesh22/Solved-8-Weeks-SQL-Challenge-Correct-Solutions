...
WITH cte_12weeks AS (
  SELECT
    ...
    SUM(CAST(sales AS BIGINT)) / SUM(CAST(transactions AS BIGINT)) AS avg_transaction_size
...
cte_results AS (
  SELECT
	...
	CAST(
	CAST(100*(LAG(total_sales) OVER (
      PARTITION BY region
      ORDER BY period_name DESC
    )-total_sales) AS NUMERIC)/CAST(total_sales AS NUMERIC) AS DECIMAL(5, 2)) AS sales_change
  FROM cte_12weeks
)
