...
SELECT 
	...
	CAST(TRIM(V1.value) AS INT)
	AS topping_id
FROM pizza_runner.pizza_recipes
CROSS APPLY STRING_SPLIT(pizza_recipes.toppings, ',') AS V1
) 
SELECT 
...
GROUP BY cte_split_pizza_names.pizza_id
ORDER BY 1;
...
SELECT 
	...
	CAST(TRIM(V2.value) AS INT) AS extras
FROM pizza_runner.customer_orders
CROSS APPLY STRING_SPLIT(customer_orders.extras, ',') AS V2
)
SELECT 
	TOP 1
...
SELECT 
	customer_orders.order_id, 
	CAST(TRIM(V3.value) AS INT) AS exclusions
FROM pizza_runner.customer_orders
CROSS APPLY STRING_SPLIT(customer_orders.exclusions, ',') AS V3
)
SELECT 
	pizza_toppings.topping_name,
	COUNT(*) AS ttl_orders
...
INNER JOIN pizza_runner.pizza_names
	ON customer_orders.pizza_id = pizza_names.pizza_id
WHERE 
	customer_orders.exclusions IS NULL
	OR
	customer_orders.extras IS NULL ORDER BY 1;
...
cte_exclusions AS (
  SELECT
    ...
    order_time,
    original_row_number,
	CAST(TRIM(V2.value) AS INT) AS topping_id
	FROM cte_cleaned_customer_orders
  CROSS APPLY STRING_SPLIT(exclusions, ',') AS V2
...
cte_extras AS (
  SELECT
    ...
    order_time,
    original_row_number,
    CAST(TRIM(V3.value) AS INT) AS topping_id
  FROM cte_cleaned_customer_orders
  CROSS APPLY STRING_SPLIT(extras, ',') AS V3
...
SELECT 
	final_output.order_id,
	final_output.customer_id,
	final_output.pizza_id,
	final_output.order_time,
	ROW_NUMBER() OVER (ORDER BY final_output.order_id) AS  original_row_number,
	final_output.ingredients_list
FROM 
(
SELECT
  order_id,
  ...
  pizza_id,
  order_time,
  original_row_number AS row_id,
  CONCAT(pizza_name, ': ', STRING_AGG(
    CASE
      WHEN topping_count > 1 THEN CONCAT(topping_count, 'x ', topping_name)
      ELSE topping_name
      END,
    ', ' 
  )) AS ingredients_list
